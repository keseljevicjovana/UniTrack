const SluzbeTable = ({ data, onDelete }) => {
  if (!data?.length) {
    return <div className="py-16 text-center text-[#8B7355] text-sm">Nema studentskih službi u sistemu.</div>;
  }
  return (
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-[#F5EFE7] border-b border-[#DDD0BE]">
            {["Naziv fakulteta", "Email", "Datum dodavanja", "Akcije"].map((h) => (
          ...
          {data.map((s) => (
            <tr key={s.id} className="border-b border-[#F0E8DC] hover:bg-[#FAF7F3] transition-colors">
              <td className="px-5 py-4">
                <div className="font-semibold text-sm text-[#2C1A0E]">{s.naziv_fakulteta}</div>
              </td>
              <td className="px-5 py-4 text-sm text-[#2C1A0E]">{s.email}</td>
              <td className="px-5 py-4 text-xs text-[#8B7355]">
                {s.datum_kreiranja ? new Date(s.datum_kreiranja).toLocaleDateString("bs-BA") : "—"}
              </td>
              <td className="px-5 py-4">
                <button
                  onClick={() => onDelete(s)}
