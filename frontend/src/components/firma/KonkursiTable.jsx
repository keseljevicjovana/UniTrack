      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-[#F5EFE7] border-b border-[#DDD0BE]">
            {["Naslov", "Pozicija", "Prijavljeno", "Rok prijave", "Datum objave", "Akcije"].map((h) => (
        ...
        <tbody>
          {data.map((k) => {
            const brojPrijava = k.broj_prijava || 0;
            const popunjeno = k.maksimalan_broj_prijava && brojPrijava >= k.maksimalan_broj_prijava;
            return (
              <tr key={k.id} className="border-b border-[#F0E8DC] hover:bg-[#FAF7F3] transition-colors">
                ...
                <td className="px-5 py-4">
                  <span
                    ...
                    style={{
                      background: popunjeno ? "#FDECEC" : "#F5EFE7",
                      color: popunjeno ? "#C62828" : "#6B4C2A",
                    }}
                  >
                    {brojPrijava} / {k.maksimalan_broj_prijava}
                  </span>
                </td>
